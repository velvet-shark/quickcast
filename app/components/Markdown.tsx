...
export function Markdown({ content, className = "" }: MarkdownProps) {
  return (
    <ReactMarkdown
      className={` bg-white w-full max-w-4xl text-md leading-relaxed ${className}`}
      ...
      components={{
        ...
        h2: ({ ...props }) => (
          <>
            <h2 className="text-3xl font-bold mb-4 mt-2" {...props} />
        ...
        code: (props) => {
          ...
          if (!isMultiLine) {
            return <code className="bg-gray-100 text-gray-800 rounded px-2 py-1 text-sm font-mono">{children}</code>;
          }
